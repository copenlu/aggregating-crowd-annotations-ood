...
def _tvd_subplot(ax, data,
    ...
    mu = data.mean()
    k = kurtosis(data)
    ax = sns.histplot(data, ax=ax, kde=True, stat='probability', color=color, line_kws={'linewidth': 3.0}, bins=20)
    ax.set_title(title)
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.text(0.98, 0.02, "K=%.2f" % k, color="black",
            ha="right", va="bottom", transform=ax.transAxes)
    ax.text(0.98, 0.1, "σ=%.2f" % sig, color="black",
            ha="right", va="bottom", transform=ax.transAxes)
    ax.text(0.98, 0.2, "μ=%.2f" % mu, color="black",
            ha="right", va="bottom", transform=ax.transAxes)
    ...
    #
    ax.set_xlim(-0.1, 1)
    ax.set_xticks(np.arange(0.0, 1.1, 0.1), labels=[f"{a:.1f}" for a in np.arange(0.0, 1.1, 0.1)])
...
def tvd_diagrams(results, num_cols=4, dpi=72, return_fig=False):

    ncols = num_cols
    nrows = (len(results) + ncols - 2) // ncols
    figsize = (ncols * 4, nrows * 4)
    ...
    for i, (plot_name, data) in enumerate(results.items()):
        q = data
        colors.append(cmap(i / len(results)))
        tvd = total_variation_distance(p, q)

        row = i // ncols
        col = i % ncols
        _tvd_subplot(ax[row, col], tvd,
                                     title="\n".join(plot_name.split()),
                                     xlabel="TVD" if row == nrows - 1 else "",
                                     ylabel="Probability" if col == 0 else "")

        #sns.histplot(tvd, ax=ax[-1,-1], shrink=0.0, kde=True, fill=None, stat='probability', color=colors[-1])
